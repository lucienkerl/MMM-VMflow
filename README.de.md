...
| `maxRefillRows` | `number\|null` | `null` | Maximale Anzahl der Produktzeilen in den Layouts `combo` und `refillProducts`. `null` (Standard) zeigt alle; bei mehr wird „+N weitere" angehängt. Pro Instanz setzbar (z. B. `combo` kappen, `refillProducts` voll lassen). |
